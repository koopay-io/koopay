		...
		async (milestoneId: string) => {
			...
			try {
				...
				if (isNetworkError(err)) {
					showNetworkError(() => fetchEvidence(milestoneId));
				} else {
					showAPIError(err, "Failed to Fetch Evidence");
				}
			} finally {
				setIsLoading(false);
			}
		...
		try {
			...
			if (uploadError) {
				...
				if (uploadError.message.includes("permission")) {
					throw new Error(
						"You don't have permission to upload evidence. Please contact support.",
					);
				}
			...
			if (insertError) {
				if (insertError.message.includes("permission")) {
					throw new Error(
						"You don't have permission to add evidence. Please contact support.",
					);
				}
				throw new Error(
			...
			if (isNetworkError(err)) {
				showNetworkError(() =>
					uploadEvidence(milestoneId, file, description),
				);
			} else {
